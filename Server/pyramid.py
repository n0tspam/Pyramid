...
def replace_assembly_line(new_line, clr_path='./Modules/clr.py'):
	"""
	Finds the line that looks like:
	assembly_bytes = b'...some hex...'
	and replaces it with new_line
	"""
	# Read the entire clr.py
	with open(clr_path, 'r', encoding='utf-8') as f:
		content = f.read()

	# This pattern looks for something like:
	# assembly_bytes = b'ANYTHING UNTIL NEXT SINGLE QUOTE'
	pattern = r"assembly_bytes\s*=\s*b'[^']*'"

	# Replace it with the new line, e.g. assembly_bytes = b'4d5a90...'
	updated_content = re.sub(pattern, new_line, content)

	# Write it back
	with open(clr_path, 'w', encoding='utf-8') as f:
		f.write(updated_content)

	print(f"[+] Updated {clr_path} with updated assembly:")
	
def get_bytes(program):
	with open(program, "rb") as f:
		...
		# Print in the b'...' format
		# Example output: assembly_bytes = b'4d5a9000...'
		
		result = f"assembly_bytes = b'{b64_data}'"
		return result
...
if __name__ == '__main__':
	...
	parser.add_argument('-targetclrbin', help=f".NET binary you wish to execute using clr.py", default="program.exe")
...
\______   \___.__.____________    _____ |__| __| _/
 ...
 Author: @naksyn
	...
	if options.targetclrbin:
		new_assembly_line = get_bytes(options.targetclrbin)

		# Now replace that line in ./Modules/clr.py
		replace_assembly_line(new_assembly_line, './Modules/clr.py')
